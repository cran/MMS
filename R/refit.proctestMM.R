...
refit.proctestMM=function(object,Ynew,z,grp,var_nonselect_fixed,var_nonselect_random,alpha,step,maxordre,choix_ordre=c("bolasso","pval","pval_hd"),m,showordre,IT,maxq,showtest,showresult,showit,speed,...)
{
	#print("refit.proctestMM")
...
q=ncol(z)
	...
	#safety
#	if(missing(m)){m=10}
#	if(missing(maxordre)){maxordre=min((ntot-1)/2,(p-1)/2)}
...
#	if(missing(maxit)){maxit=20}#nombre de fois ou on redemarre l'algo
#	if(missing(maxq)){maxq=log(min(ntot,p)-1,2)}
...
if(missing(var_nonselect_fixed)) var_nonselect_fixed=object$arg$var_nonselect_fixed
if(missing(var_nonselect_random)) {var_nonselect_random_sauv=object$arg$var_nonselect_random_sauv}else{var_nonselect_random_sauv=var_nonselect_random}
if(missing(maxordre)) maxordre=object$arg$maxordre
if(missing(choix_ordre)) choix_ordre=object$arg$choix_ordre
if(missing(m)) m=object$arg$m
if(missing(showordre)) showordre=object$arg$showordre
if(missing(IT)) IT=object$arg$IT
if(missing(maxq)) {maxqdep=object$arg$maxqdep}else{maxqdep=maxq}
if(missing(showtest)) showtest=object$arg$showtest
if(missing(showresult)) showresult=object$arg$showresult
if(missing(showit)) showit=object$arg$showit
...
aV2[,,1:dim(object$quantile)[3],1]=object$quantile
	...
	#aV2=array(0,c(length(alpha),dim(object$quantile)[2],dim(object$quantile)[3],nrow(object$ordrebeta))) 	
	#aV2[,,,1:nrow(object$ordrebeta)]
	aV2=object$quantile
}
...
mm2=MM2(data2=data2,yhat=Y,m=m,sigma=object$sigma_e+sum(object$sigma_u),maxqdep=maxqdep,maxordre=maxordre,var_nonselect=var_nonselect,random=random,showtest=FALSE,showit=showit,showresult=showresult,correspondance=correspondance,ORDREBETA2=ORDREBETA2,indice=indice2,aV2=aV2,choix_ordre=choix_ordre,alpha=alpha,alph=alph,IT=IT)
...
#mean that data2[ordre]=data[correspondance[2,ordre]]=data[,bb]
	...
	#on ne calcule que les manquants
			#source("MM2.R")
mm2=MM2(data2=data2,yhat=yhat,m=m,sigma=sigma_e,maxqdep=maxqdep,maxordre=maxordre,var_nonselect=var_nonselect,random=random,showtest=FALSE,showit=showit,showresult=showresult,correspondance=correspondance,ORDREBETA2=ORDREBETA2,indice=indice2,aV2=aV2,choix_ordre=choix_ordre,alpha=alpha,alph=alph,IT=IT)
...
out=list(data=list(X=data,Y=Y,z=z,grp=grp),beta=BETA_HAT,fitted.values=y.fitted,u=UCHAP,sigma_u=SIGMAU,sigma_e=SIGMAE,it=COMPTEUR,quantile=aV2,ordrebeta=ORDREBETA2,converge=converge,call=match.call(),arg=object$arg)
